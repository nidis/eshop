...
import com.nidis.eshop.services.SessionService;
...
public class CartController {
    ...
    @Autowired
    public CartController(CartService cartService, CartItemService cartItemService, SessionService sessionService, CartAssembler cartAssembler) {
    ...
    public ResponseEntity<?> getCart(@PathVariable Long cartId, HttpServletRequest req) {
        String sessionId = getCookie(req, CART_SESSION_ID);
        String ipAddress = req.getLocalAddr();

        Optional<Session> session = sessionService.findByIdAndIpAddress(Long.parseLong(sessionId), ipAddress);
